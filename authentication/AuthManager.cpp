                                     ...
                                     uint64_t expiry)
{
    ...
    session->auth_state.store(
        ...
        std::memory_order_release);
        // remove temporary userid to real user id and update all information we kept behalf of userid such as id->socket and socket->id


        session->userId=UserIdToHexString(user_id);
}
