...
{
    Packet p;
    p.serialize(PKT_OTP_VERIFY, email, otp, "");
    return sendRawPacket(p);
}
...
bool Client::login(const std::string& identifier, const std::string& password)
{
    userId = identifier;
    Packet p;
    p.serialize(PKT_LOGIN, identifier, password, "");
    return sendRawPacket(p);
}
...
bool Client::sendFile(const std::string& receiver, const std::string& filepath) {
    ...
    std::string payload = std::to_string(totalSize) + " "
                        + fileName + " "
                        + uploadId + " "
...
void Client::handleFileChunk(Packet& p) {
    ...
    if (p.header.size < HEADER_SIZE + sizeof(ChunkHeader)) return;
...
void Client::handleRoundStatus(Packet& p) {
    ...
    pos++; // skip recid
    while (pos < raw.size() && raw[pos] != ' ') missingData.push_back(raw[pos++]);
...
void Client::HandleFileStatus(Packet& p) {
    ...
    while (pos < raw.size() && raw[pos] != ' ') pos++;
    pos++; // skip recid
    while (pos < raw.size() && raw[pos] != ' ') message.push_back(raw[pos++]);
